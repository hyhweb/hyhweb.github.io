...
(function($){
	...
	function setSize(target) {
		...
		$('.datagrid-view1 .datagrid-header table th',grid).height(hh);
		$('.datagrid-view2 .datagrid-header table th',grid).height(hh);
		header1.height(hh);
		header2.height(hh);
	...
	function fixColumnSize(target, cell) {
		...
		function fix(cell) {
			...
			//col.width = $.boxModel==true ? headerCell.width() : headerCell.width();
	...
	$.fn.datagrid = function(options, param){
		...
		return this.each(function(){
			...
			if (opts.toolbar) {
				var tb = $('<div class="datagrid-toolbar clearfix"></div>').prependTo($('.datagrid-wrap', grid));
