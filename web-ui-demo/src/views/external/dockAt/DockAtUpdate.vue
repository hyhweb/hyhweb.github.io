<template>
  <el-dialog
    ...
    <el-form ref="form" :rules="addFormRules" :model="form"  label-width="150px">
      ...
      <el-form-item label="码头(安通)编码"  prop="contrastcode">
        <el-input v-model="form.contrastcode"></el-input>
      </el-form-item>
      <el-form-item label="码头(安通)名称"  prop="contrastname">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        addFormRules: {
          ...
          contrastcode: [
            { required: true, message: '请输入码头(安通)编码',trigger:'blur,change' }
          ],
          contrastname: [
            { required: true, message: '请输入码头(安通)名称',trigger:'blur,change' }
          ],
        },
        form: {
          id :'',
    ...
    methods: {
      ...mapActions([
        'getDockAtList','updateDockAt','getalldockList'
      ]),
      getList(){
          ...
          this.$refs.form.validate((valid) => {
            if(valid){
              this.form.dockName = this.alldockList.find(item => item.dockId === this.form.dockId)['name'];
                this.updateDockAt(this.form).then(res =>{
                  if(res.success){
                    this.$refs.form.resetFields()
                    this.newDockdialogVisible = false;
                      this.getDockAtList();
