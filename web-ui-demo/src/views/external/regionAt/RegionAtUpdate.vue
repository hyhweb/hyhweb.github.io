...
<el-dialog
  ...
  <el-form ref="form" :rules="addFormRules" :model="form"  label-width="200px">
    ...
    <el-form-item label="地区(安通)编码"  prop="contrastcode">
      <el-input v-model="form.contrastcode"></el-input>
    </el-form-item>
    <el-form-item label="地区(安通)名称"  prop="contrastname">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        addFormRules: {
          ...
          contrastcode: [
            { required: true, message: '请输入地区(安通)编码',trigger:'blur,change' }
          ],
          contrastname: [
            { required: true, message: '请输入地区(安通)名称',trigger:'blur,change' }
          ],
        },
        detail:{},
        selectData:[],
    ...
    methods: {
      ...mapActions([
        'updateRegionAt', 'getRegionAtList','getRegions','RegionBelongPList','RegionBelongCList','getRegionRecursiveByRegionId'
        ]),
      getList(){
      ...
      onSubmit() {
          ...
          if(valid){
            ...
            if(this.form.selectedRegionFour){
             ...
             this.updateRegionAt(this.form).then(res =>{
               if(res.success){
                    this.$refs['form'].resetFields();
                    this.newRegiondialogVisible = false;
