<template>
  <el-dialog
    ...
    <el-form ref="form" :rules="addFormRules" :model="form"  label-width="200px">

      <!--<el-form-item  label="地区(运柜宝)名称" prop="regionCode">-->
        <!--<el-select filterable clearable v-model="form.regionCode" placeholder="请选择地区(运柜宝)地区(运柜宝)" >-->
      ...
      <el-form-item label="(运柜宝)地区(安通)编码"  prop="contrastcode">
        <el-input v-model="form.contrastcode"></el-input>
      </el-form-item>
      <el-form-item label="(运柜宝)地区(安通)名称"  prop="contrastname">
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        addFormRules: {
          selectedRegion: [
            { required: true, message: '请选择(运柜宝)地区(运柜宝)名称',trigger:'blur,change' }
          ],
          contrastcode: [
            { required: true, message: '请输入(运柜宝)地区(安通)编码',trigger:'blur,change' }
          ],
          contrastname: [
            { required: true, message: '请输入(运柜宝)地区(安通)名称',trigger:'blur,change' }
          ]
        },
        second:false,
        third:false,
...
//        'regionAllList'
    ...
    methods: {
      ...mapActions([
        'getRegionAtList','saveRegionAt', 'RegionBelongPList','RegionBelongCList'
//        ,'getAllRegionList'
      ]),
      selectRegionaction(){
...
//
      ...
      onSubmit() {
        ...
        this.$refs.form.validate((valid) => {
          if(valid){
            ...
            this.saveRegionAt(this.form).then(res =>{
              if(res.success){
                this.$refs['form'].resetFields();
                this.newRegiondialogVisible = false;
                this.$emit('closeSave');
    ...
    created(){
//      this.$store.dispatch('getAllRegionList');
