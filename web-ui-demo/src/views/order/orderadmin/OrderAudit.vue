<template>
  <section>
    <el-dialog :visible.sync="orderModel" size="full" id="order-audit">
      <!--订单详情-->
      <el-col>
      ...
      <el-col>
        <el-tabs v-model="tabsSelected">
          <el-tab-pane label="柜子信息" name="first">
            <el-table :data="orderDetail.cabinetList">
        ...
        <el-tabs v-model="tabsSelected">
          <el-tab-pane label="单证信息" name="first">
            <el-table :data="documentInfo">
              ...
              <el-table-column prop="realRequireDate" label="实际装送货时间">
              </el-table-column>
              <el-table-column prop="" label="">
              </el-table-column>
            </el-table>
      ...
      <el-col class="textAlign bottomfixed" v-bind:style="bottomfixed">
        ...
        <el-button type="primary" @click="confirmSubmit">确定订单</el-button>
        <!--<el-button type="danger" @click="cancelSubmit">放弃订单</el-button> -->
...
export default {
  ...
  computed: {
    ...
    //箱量统计
    cabinetCount() {
      if (typeof(this.orderDetail.cabinetCountMap) == "undefined" || this.orderDetail.cabinetCountMap == null) {
        return []
      }
      return [{
  ...
  methods: {
    ...mapActions([
      'getOrderDetail', 'getOrderApproval'
    ]),
    remarkShowHandle() {
      if (this.remarkShow) {
    ...
    confirmSubmit() {
      ...
      if (this.orderDetail.orderType == 1) {
        this.orderDetail.cabinetList.map((val, key) => {
          if (val.cabinetNo == "" || val.cabinetNo == null) {
            this.$message({
              message: '箱号不能为空，请填写箱号再审核',
              type: 'warning'
            })
            valide = true
            return;
      ...
      if (this.orderDetail.requireDate == "" || this.orderDetail.requireDate == null) {
        let titleTip = this.orderTypeTimeTitle + "不能为空";
        this.$message({
          message: titleTip,
          type: 'warning'
        })
      ...
      if (this.orderDetail.contactTime == "" || this.orderDetail.contactTime == null) {
      ...
      if (valide) return
      this.$confirm('确定审核通过订单？').then(() => {
        let param = { orderId: this.orderId, type: '1' }
        this.getOrderApproval(param).then((res) => {
          if (res.success) {
    ...
    cancelSubmit() {
      this.$confirm('确定放弃订单？').then(() => {
        let param = { orderId: this.orderId, type: '0' }
        this.getOrderApproval(param).then((res) => {
    ...
    onScroll1() {
      this.$nextTick(function() {
        let scrolled = document.getElementById("order-audit").childNodes[0].scrollTop
  ...
  mounted() {
    document.getElementById("order-audit").childNodes[0].addEventListener('scroll', this.onScroll1);
