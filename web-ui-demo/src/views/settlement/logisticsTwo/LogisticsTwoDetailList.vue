<template>
  <section>
    ...
    <el-col :span="24" class="toolbar">
      ...
      <el-button type="primary" icon="document" @click="exportLogisticsTwoTemplate" size="small" :disabled="this.$global.isDisabled(21)">导出Excel(应付匹配用)</el-button>
      <div style="float:right;margin-top:8px;margin-right:10px">
        当前页应付(元)：<span style="color:red">{{total}}</span> &nbsp;&nbsp;
        总应付(元)：<span style="color:red">{{twoDetailAmount}}</span>
      </div>
    </el-col>
    ...
    <el-col>
      <el-table
        :data="logisticsTwoDetail.data" :maxHeight="tableMaxHeight" stripe
      >
        <el-table-column
          prop="subName"
          label="子公司" width="100">
...
<script>
  ...
  import {getLogisticsTwoDetailExcelApi,exportLogisticsTwoDetailRequisitionTemplateApi} from '../../../api/modules/settlement';
  import Qs from 'qs';
  ...
  export default {
    ...
    computed: {
      ...mapGetters([
        'logisticsTwoDetail', 'logisticsTwoDetailAmount'
      ]),
      total(){
        let alltotal = 0;
        if (this.logisticsTwoDetail.data) {
          this.logisticsTwoDetail.data.forEach((val, key) => {
            if (val.total != null) {
    ...
    methods: {
      ...mapActions([
        'getLogisticsTwoDetail', 'getLogisticsTwoDetailAmount'
      ]),
      init(){
        this.getList();
      ...
      putDispatch(){
        ...
        this.download({
          params: params,
          handle: getLogisticsTwoDetailExcelApi,
          callback () {
      ...
      exportLogisticsTwoTemplate(){
        this.$confirm('确认要导出应付匹配模板吗?', '提示', {
          ...
          this.download({
            params: params,
            handle: exportLogisticsTwoDetailRequisitionTemplateApi,
            callback () {
      ...
      //获取列表
      getList(initpage){
        let log = util.localStorage.get('logisticsTwo');
        ...
        let success = success =>{
          this.twoDetailAmount = this.logisticsTwoDetailAmount;
        };
        ...
        this.pageHandler(this.form, this.getLogisticsTwoDetail,initpage,success);
      },
      //获取总金额
      getDetailAmount(){
        this.getLogisticsTwoDetailAmount(this.form)
      },
      //查询
      onSubmit() {
