<template>
  <section>
        ...
        <el-form class="query-modal-lg" :inline="true" :model="form" ref="form" @submit.prevent="onSubmit">
          <div class="query-params">
            <el-form-item label="子公司" :label-width="labelWidth" prop="childCompanyCode">
              <el-select
                :disabled="isSearchChildCompany"
                clearable
                filterable
                default-first-option
                v-model="form.childCompanyCode" placeholder="请选择子公司" class='control-width'>k
                <el-option
    ...
    <el-col>
      <el-table
        ref="table" @row-click="handleRowClick" :maxHeight="tableMaxHeight" :data="customerDetail.data" stripe
      >
...
<script>
  ...
  import {getCustomerDetailExcelApi, getDeptmentListApi} from '../../../api/modules/settlement';
  import Qs from 'qs';
  ...
  export default {
    ...
    computed: {
      ...mapGetters([
        'customerDetail', 'customerDetailAmount'
      ]),
      //进行统计当页的总金额
      total(){
        let alltotal = 0;
        if (this.customerDetail.data) {
          this.customerDetail.data.forEach((val, key) => {
            if (val.total != null) {
    ...
    methods: {
      ...mapActions([
        'getCustomerDetail', 'getCustomerDetailAmount'
      ]),
      init(){
        this.getList();
        this.getCusAmount();
      ...
      //获取列表
      getList(initpage){
        let user = util.localStorage.get('customer');
        if(user.orgCode != '' && user.orgCode != null){
            this.form.childCompanyCode = user.orgCode;
            this.isSearchChildCompany = true;
        }
        this.form.customerId = user.objectId;
        this.form.costStatus = user.costStatus;
        this.form.currentData = user.endDate;
        this.form.settlementDateStart = user.settlementDateStart;
        this.form.settlementDateEnd = user.settlementDateEnd;
        this.form.endDateStart = user.endDateStart;
        this.form.endDateEnd = user.endDateEnd;
        let success = success =>{
          this.detailTotalAmount = this.customerDetailAmount;
        };
        ...
        this.pageHandler(this.form, this.getCustomerDetail,initpage,success)
      },
      //导出excel
      putDispatch(){
        this.$confirm('确认要导出任务吗?', '提示', {
          ...
          this.download({
            params:params,
            handle: getCustomerDetailExcelApi,
            callback () {
            }
          });
        });
      ...
      //统计所以数据的总金额
      getCusAmount(){
        this.getCustomerDetailAmount(this.form)
      },
      goBack(){
        this.$router.back(-1)
      },
