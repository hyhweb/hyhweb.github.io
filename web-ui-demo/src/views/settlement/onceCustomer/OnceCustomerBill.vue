  ...
  <section>
    <ToolBarBox>
      ...
      <el-col :span="24" class="toolbar">
        <div style="display: inline-block;padding: 3px;border-radius: 8px;box-shadow: inset 1px 1px 1px #d1dbe5, inset 0 0 70px #d1dbe5;">
          <el-button type="primary" icon="document" v-if="!this.$global.isDisabled(73)" :disabled="selectData.length!=1" @click="onceAndReceiveExportExcel" size="small">导出账单</el-button>
          <el-checkbox v-model="isContainReceivableCost">包含已申请费用</el-checkbox>
        </div>
        &nbsp;&nbsp;
    ...
    <TableBox>
      ...
      <el-col>
        <el-table ref="table" @row-click="handleRowClick" :maxHeight="tableMaxHeight"
                  :data="onceCustomerBill.data" stripe @selection-change="handleSelectionChange"
        >
    ...
    <OnceCustomerReceivableDetail ref="onceCustomerReceivableDetail"></OnceCustomerReceivableDetail>
  </section>
</template>
<script>
  ...
  import OnceCustomerReceivableDetail from './OnceCustomerReceivableDetail.vue'
  import {getOnceCustomerBillAmountApi,exportOnceCustomerBillApi,
    getDeptmentListApi,onceCusAddReceivableApi} from '../../../api/modules/settlement';
  import Qs from 'qs';
  export default {
    mixins: [pagination,download],
    components: {
      OnceCustomerReceivableDetail
    },
    data() {
      return {
        labelWidth: '80px',
    ...
    computed: {
      ...mapGetters([
        'onceCustomerBill', 'customerAmount'
      ]),
      //进行统计当页的总金额
      total(){
        if (JSON.stringify(this.onceCustomerBill.data) == "") {
          this.buttonStatus = true;
        }
        let alltotal = 0;
        if (this.onceCustomerBill.data) {
          this.onceCustomerBill.data.forEach((val, key) => {
            if (val.total != null) {
    ...
    methods: {
      ...mapActions([
        'getOnceCustomerBillList', 'getCustomerAmount'
      ]),
      getAmount(list, code){
        if (list) {
...
//        }
        ...
        this.$global.recorderTableData(this, 'onceCustomerBill',this.getOnceCusAmount(this.form));
      ...
      onceAndReceiveExportExcel(){
        if (this.selectData.length >= 1) {
          this.$confirm('确认要账单吗?', '提示', {
            ...
            this.download({
              params: params,
              handle: exportOnceCustomerBillApi,
              message: {
      ...
      addReceivableRequisition(){
        if (this.selectData.length === 1) {
          ...
          this.$confirm('确认要生成收款单吗?', '提示', {
            ...
            param.endDateStart = this.endDateStart_cache;
            param.endDateEnd = this.endDateEnd_cache;
            onceCusAddReceivableApi({data:JSON.stringify(param)}).then(res=>{
      ...
      getCustomerReceivableDetail(){
        if (this.selectData.length === 1) {
          ...
          this.$refs.onceCustomerReceivableDetail.detailVisible = true;
          this.$refs.onceCustomerReceivableDetail.form.showFlag=true;
          this.$refs.onceCustomerReceivableDetail.form.receivableRequisitionId = this.selectData[0].receivableRequisitionId;
          this.$refs.onceCustomerReceivableDetail.init()
        }else{
          this.$refs.onceCustomerReceivableDetail.detailVisible = false;
          this.$refs.onceCustomerReceivableDetail.form.showFlag=false;
      ...
      goCusDetails(res){
        ...
        util.localStorage.set('onceCus', res);
        this.$router.push('/accounting/temporary-customer/receivable/Detail')
      },
      // 显示或隐藏操作按钮
      showButtons(){
      ...
      getList(initpage){
        ...
        this.pageHandler(this.form, this.getOnceCustomerBillList, initpage);


      },
      //统计所有数据的总金额
      getOnceCusAmount(val){
          getOnceCustomerBillAmountApi(val).then(res => {
            if(res.data.code === '1'){
              this.totalAmount = res.data.data.toFixed(2);
            }
      ...
      //查询
      onSubmit() {
        this.getOnceCusAmount(this.form);
        this.getList(true)
