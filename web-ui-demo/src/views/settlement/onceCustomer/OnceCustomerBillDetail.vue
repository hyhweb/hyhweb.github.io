<template>
  <section>
    ...
    <el-col>
      <el-table
        ref="table" @row-click="handleRowClick" :maxHeight="tableMaxHeight" :data="onceCustomerBillDetail.data" stripe
      >
...
<script>
  ...
  import {getOnceCustomerBillDetailAmountApi, getDeptmentListApi} from '../../../api/modules/settlement';
  import Qs from 'qs';
  ...
  export default {
    ...
    computed: {
      ...mapGetters([
        'onceCustomerBillDetail', 'customerDetailAmount'
      ]),
      //进行统计当页的总金额
      total(){
        let alltotal = 0;
        if (this.onceCustomerBillDetail.data) {
          this.onceCustomerBillDetail.data.forEach((val, key) => {
            if (val.total != null) {
    ...
    methods: {
      ...mapActions([
        'getOnceCustomerBillDetailList', 'getCustomerDetailAmount'
      ]),
      init(){
        this.getList();
//        this.getCusAmount();
      ...
      //获取列表
      getList(initpage){
        let user = util.localStorage.get('onceCus');
        ...
        this.pageHandler(this.form, this.getOnceCustomerBillDetailList,initpage,()=>{
      ...
      //统计所以数据的总金额
      getCusAmount(val){
        getOnceCustomerBillDetailAmountApi(val).then(res=>{
          if(res.data.code === '1'){
            this.detailTotalAmount = res.data.data.toFixed(2);
          }
        })
      },
      goBack(){
