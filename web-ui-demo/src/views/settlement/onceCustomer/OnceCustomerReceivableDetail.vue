  ...
  <el-dialog
    ...
    <el-col>
      ...
      <el-col v-show="form.showFlag">
        <el-table ref="table" @row-click="handleRowClick" :data="onceCustomerReceivableDetailListData" @selection-change="handleSelectionChange" stripe>
...
<script>
  ...
  import {exportOnceCustomerReceivableDetailApi,onceCusGetReceivableDetailAmountApi} from '../../../api/modules/settlement'
  ...
  export default {
    ...
    computed: {
      ...
      onceCustomerReceivableDetailListData(){
        if (this.onceCustomerReceivableDetailList == undefined) {
          return []
        } else {
          return this.onceCustomerReceivableDetailList.data
        }
      }
    },
    methods: {
      ...mapActions([
        'getOnceCustomerReceivableDetailList'
      ]),
...
//        console.log(this.receivableRequisitionDetailList.data);
        ...
        this.pageHandler(this.form, this.getOnceCustomerReceivableDetailList,false,()=>{
            this.getDetailSumAmount(this.form)
        })
      },
      closeDialog() {
...
//        }
      ...
      exportExcel2(){
        this.$confirm('确认要导出任务吗?', '提示', {
          ...
          this.download({
            params: params,
            handle: exportOnceCustomerReceivableDetailApi,
            callback () {
            }
          })
        });
      ...
      getDetailSumAmount(val){
        onceCusGetReceivableDetailAmountApi(val).then((res) => {
          let data = res.data;
          if (data.code === '1') {
