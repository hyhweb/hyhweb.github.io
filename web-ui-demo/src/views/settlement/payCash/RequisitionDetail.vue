  ...
  <el-col>
    ...
    <el-col :span="24" class="toolbar" style="margin-bottom:0px;">
      <div style="float:left;margin-right:10px">
        <span style="font-family: Microsoft YaHei;font-size: 120%">杂费报销详情</span>
      </div>
...
<script>
  ...
  import {getDetailSumAmount1Api} from '../../../api/modules/settlement'
  import {getRequisitionExcelApi} from '../../../api/modules/settlement'
  import {getPayCashExcelDetailExcelApi} from '../../../api/modules/settlement'
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['payCashRequestDetailList']),
      requisitiondetailListData(){
        if (this.payCashRequestDetailList.data == undefined) {
          return []
        } else {
          return this.payCashRequestDetailList.data
        }

      }
    },
    methods: {
      ...mapActions(['outPortPayCash', 'getPayCashRquestDetail']),
      getList(){
//订单号
...
//箱号
        ...
        this.pageHandler(this.form, this.getPayCashRquestDetail)
...
//导出excel
      exportExcel2(){
        this.$confirm('确认要导出任务吗?', '提示', {
          ...
          this.download({
            params: params,
            handle: getPayCashExcelDetailExcelApi,
            callback () {
...
//获取总金额
      getDetailSumAmount(val){
        let param = {reqId: val}
        getDetailSumAmount1Api(param).then((res) => {
          let data = res.data;
          if (data.code == 1) {
...
//第二种方法(Json提交)
              ...
              this.outPortPayCash({data: JSON.stringify(this.RequisitionFeesForm)}).then(res => {
                if (res.success) {
                  this.getList();
                  this.$parent.getList();
                  this.$message({
