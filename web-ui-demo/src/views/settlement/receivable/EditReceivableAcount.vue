    ...
    <el-col>
        ...
        <el-form class="query-modal-lg" :inline="true" :model="form" ref="form" @submit.prevent="onSubmit">
          <div class="query-params">
            ...
            <el-form-item label="应收对象" :label-width="labelWidth" prop="customerName">
              <el-input v-model="customerName" placeholder="请输入应收对象" class='control-width' :disabled="true"></el-input>
            </el-form-item>
            ...
            <el-form-item label="行程状态" :label-width="labelWidth" prop="scheduleStatus">
              <el-select
                clearable
                filterable
                default-first-option
                ...
                <el-option label="已完成" :key="form.scheduleStatus" :value="5"></el-option>
                <el-option label="未完成" :key="form.scheduleStatus" :value="1"></el-option>
              </el-select>
            </el-form-item>
            ...
            <div style="display: inline-block;">
              <el-form-item label="行程结束日期" :label-width="labelWidth" prop="endDateStart" style="margin-right:2px">
                <el-date-picker type="date" icon="" placeholder="选择日期" v-model="form.endDateStart"></el-date-picker>
              </el-form-item>
              ...
              <el-form-item label="" :label-width="labelWidth" prop="endDateEnd">
                <el-date-picker type="date" placeholder="选择日期" v-model="form.endDateEnd"></el-date-picker>
              </el-form-item>
            </div>
            <div style="display: inline-block;">
              <el-form-item label="结算日期" :label-width="labelWidth" prop="settlementDateStart" style="margin-right:2px">
                <el-date-picker type="date" icon="" placeholder="选择日期" v-model="form.settlementDateStart"></el-date-picker>
              </el-form-item>
              <label style="padding-right:0;padding-top:11px;display: inline-block;">-</label>
              <el-form-item label="" :label-width="labelWidth" prop="settlementDateEnd">
                <el-date-picker type="date" placeholder="选择日期" v-model="form.settlementDateEnd"></el-date-picker>
              </el-form-item>
            </div>
      ...
      <el-col :span="24" class="toolbar" style="margin-bottom:0px;">
        ...
        <div style="float:right;margin-top:8px;margin-right:10px">
          选中：<span style="color:red">{{ totalSelect_daiyinru }}</span>条&nbsp;&nbsp;&nbsp;
          金额：<span style="color:red">{{ totalAmount_daiyinru }}</span>元
        </div>
      </el-col>

      <!-- 编辑应收款.待引入.查询结果-->
      <el-col>
        <el-table ref="table" @row-click="handleRowClick" :data="receivableImportPageListData" @selection-change="handleSelectionChange" stripe border>
          ...
          <el-table-column
            prop="settlementDate"
            label="结算日期"  width="160">
          </el-table-column>
          <el-table-column
            prop="endDate"
            label="行程结束日期"  width="160">
          </el-table-column>
        </el-table>
      </el-col>
      ...
      <el-col :span="24" style="text-align: center;line-height: 40px;">
        ...
        <el-button type="primary" icon="arrow-down" @click="importReceivableAcount" size="small" :disabled="this.$global.isDisabled(104) || requisitionStatus === 6"></el-button>
        <el-button type="primary" icon="arrow-up" @click="outportReceivableAcount" size="small" :disabled="this.$global.isDisabled(105) || requisitionStatus === 6"></el-button>
      </el-col>


      <!-- 编辑应收款.已引入.标题-->
      <Yiyinru ref="yiyinru"></Yiyinru>
...
<script>
  ...
  import Yiyinru from './EditReceivableAcount2.vue'
...
//          orgName: ''
    ...
    computed: {
      ...
      receivableImportPageListData(){
        if (this.receivableImportPageList == undefined) {
          return []
        } else {
          return this.receivableImportPageList.data
        }
      },
      totalSelect_daiyinru() {
        return this.selectData.length;
      },
      totalAmount_daiyinru() {
        let totalAmount_daiyinru = 0;
        this.selectData.forEach(function(item) {
          //totalAmount_daiyinru += parseInt(item.incomeAmount);
          if(item.incomeAmount) {
            totalAmount_daiyinru += item.incomeAmount;
          }
        });
...
//            "requisitionId":this.form.receivableRequisitionId,	//申请单Id/编号
            "requisitionId":this.receivableRequisitionId2,	//申请单Id/编号
            "importDtoList":[]
          };
//          let paramDetail = {
//            "customerId": "",	//收款对象ID
...
//          console.log(param)
          importReceivableAcountApi({data: JSON.stringify(param)}).then((res) => {
            if (res.data.success) {
              this.$emit("importReceivableAcountSuccess");
              this.getList();
              this.$message({
                message: '引入成功',
                type: 'success'
      ...
      outportReceivableAcount() {
        if (this.$refs.yiyinru.selectData.length != 0) {
          //参数格式的封装
          let param = {
//            "reqId":this.form.receivableRequisitionId,		//申请单Id
            "reqId":this.receivableRequisitionId2,		//申请单Id
            "reqDetailIds":[],	//申请单明细Id
            "cabCostIds":[]	//订单费用Id
          };
          this.$refs.yiyinru.selectData.forEach(function(item) {
          ...
          outportReceivableAcountApi({data: JSON.stringify(param)}).then((res) => {
            if (res.data.success) {
              this.$emit("outportReceivableAcountSuccess");
              this.getList();
              this.$message({
                message: '移出成功',
                type: 'success'
      ...
      getList(){
        //1.获取费用类型、子公司 的下拉列表数据
        this.getCostTypeList();
        this.getChildCompany()

        //2获取应收款-待引入 的表格数据
//        console.log("获取待引入的表格数据："+this.form.customerId)
//        this.pageHandler({
//          "customerId": this.form.customerId
//        }, this.getReceivableImportPageList)
//        console.log(this.form)
        this.pageHandler(Object.assign(this.form, { receivableRequisitionId: '' }, { customerName: this.customerName }), this.getReceivableImportPageList)
...
//        }, this.getReceivableRequisitionDetailList)
        ...
        this.$nextTick(() => {
//          console.log("获取已引入的表格数据："+this.form.customerId)
          this.$refs.yiyinru.form.receivableRequisitionId = this.receivableRequisitionId2;
          this.$refs.yiyinru.getList();
        })
...
//        this.getSumAmount()
        ...
        this.form.settlementDateStart = (typeof(this.form.settlementDateStart) == "string" ? this.form.settlementDateStart : util.formatDate.format(this.form.settlementDateStart, 'yyyy-MM-dd hh:mm:ss'));
        this.form.settlementDateEnd = (typeof(this.form.settlementDateEnd) == "string" ? this.form.settlementDateEnd : util.formatDate.format(this.form.settlementDateEnd, 'yyyy-MM-dd hh:mm:ss'));
        this.form.endDateStart = (typeof(this.form.endDateStart) == "string" ? this.form.endDateStart : util.formatDate.format(this.form.endDateStart, 'yyyy-MM-dd hh:mm:ss'));
        this.form.endDateEnd = (typeof(this.form.endDateEnd) == "string" ? this.form.endDateEnd : util.formatDate.format(this.form.endDateEnd, 'yyyy-MM-dd hh:mm:ss'));
